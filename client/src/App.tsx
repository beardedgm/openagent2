...
const ResourceHubPage = lazy(() =>
  import('./pages/ResourceHubPage').then((m) => ({ default: m.ResourceHubPage })),
);
const TaskDetailPage = lazy(() =>
  import('./pages/TaskDetailPage').then((m) => ({ default: m.TaskDetailPage })),
);
const TaskEditorPage = lazy(() =>
  import('./pages/TaskEditorPage').then((m) => ({ default: m.TaskEditorPage })),
);
...
export function App() {
  return (
    // Render redeploys invalidate hashed chunk URLs; without a boundary a stale session's next
    // lazy navigation would white-screen. The primary route Suspense lives inside AppShell
    // (around its Outlet) so the shell chrome stays stable; this outer Suspense only covers
    // login/register and the shell boundary itself.
    <ChunkErrorBoundary>
